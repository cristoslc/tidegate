...
> - ADR-005 (Accepted) makes the agent boundary structural via microVM, eliminating the need for the runtime to supply a process boundary
> - ADR-004 (Accepted) revealed that ADR-003's claimed advantage (filesystem IPC = no dual-homing) was actually an unscanned exfiltration path
> - The system architecture evolved to a runtime-agnostic model with three integration modes based on scanning fidelity, not runtime identity
...
| Superseded | 2026-03-11 | — | Runtime selection no longer operative; ADR-005 (microVM isolation) makes the agent boundary structural, and the system architecture's runtime-agnostic model replaces runtime picking |
